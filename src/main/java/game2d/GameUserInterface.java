...
// RPG version of draw	
	...
	private void drawTitleScreen() {
		...
		int scWidth = gp.getScreenWidth();
		int scHeight = gp.getScreenHeight();
		
		// background
		g2.setColor(new Color(0, 0, 0));
		g2.fillRect(0,  0, scWidth, scHeight);
		g2.setColor(Color.CYAN);
		for (int counterJ = 0; counterJ < 4; counterJ++) {
			int drawY = (scHeight / 2) + (75 * counterJ);
		    g2.drawLine(0, drawY, scWidth, drawY);
		}
		for (int counterK = 0; counterK < 8; counterK++) {
			g2.drawLine(75 - (scWidth / 2) + (150 * counterK), scHeight, 75 + (150 * counterK), (scHeight / 2));
		}
		
		// title
		g2.setFont(g2.getFont().deriveFont(Font.BOLD,72F));
		String text1 = "Cassandra";
		String text2 = "Grid";
